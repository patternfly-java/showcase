...
public class BadgeComponent extends BaseComponent {
    ...
    public BadgeComponent() {
        super("Badge", p().textContent(
                ...
                .element(),
                asList(new Snippet("Badge (read)", "Resources.get().badgeUnread().getText()",
                        () -> div().add(Badge.read(7)).add(" ").add(Badge.read(24)).add(" ").add(Badge.read(423)).add(" ")
                                .add(Badge.read("999+")).element()),
                        new Snippet("Badge (unread)", "Resources.get().badgeRead().getText()",
                                () -> div().add(Badge.unread(7)).add(" ").add(Badge.unread(24)).add(" ").add(Badge.unread(423))
                                        .add(" ").add(Badge.unread("999+")).element())));
    }
}
