...
public class EmptyStateComponent extends BaseComponent {
    ...
    public EmptyStateComponent() {
        super("Empty state", p().textContent("Use an empty state component when there is no data or other information to show. "
                + "It can also be used to communicate completion or other status at the end of a sequential "
                + "task. An empty state may include a call to action to guide the user on what to do next.").element(), asList(
                        new Snippet("Empty state", "Resources.get().emptyState().getText()", () -> div()
                                ...
                                .element()),
                        new Snippet("Empty state small", "Resources.get().emptyStateSmall().getText()", () -> div()
                                .add(emptyState(icon(fas("cubes")), "Empty State").small()
                                        ...
                                        .secondary("Action area", noop()).element())
                                .element()),
                        new Snippet("Empty state large", "Resources.get().emptyStateLarge().getText()", () -> div()
                                .add(emptyState(icon(fas("cubes")), "Empty State").large()
                                        ...
                                        .secondary("Action area", noop()).element())
                                .element()),
                        new Snippet("Empty state w/ primary element", "Resources.get().emptyStatePrimary().getText()",
                                () -> div()
                                        .add(emptyState(icon(fas("cubes")), "Empty State")
                                                .body("This represents an the empty state pattern in PatternFly 4. "
                                                        + "Hopefully it's simple enough to use but flexible enough to "
                                        ...
                                        .element()),
                        new Snippet("Empty state loading", "Resources.get().emptyStateSpinner().getText()",
                                () -> div().add(EmptyState.spinner()).element()),
                        new Snippet("Empty state no match found", "Resources.get().emptyStateNoMatch().getText()",
